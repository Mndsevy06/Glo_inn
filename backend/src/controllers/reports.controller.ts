...
export const getKpis = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const toNum = (d: any) => parseFloat(d?.toString() ?? '0');
  ...
  try {
    ...
    const results = await Promise.all(
      days.map(async (day) => {
        ...
        const express = commandes.reduce((s, c) =>
          s + c.lignes.filter(l => l.type_service === 'Express').reduce((ss, l) => ss + toNum(l.sous_total), 0), 0);
...
// ─── TOP SERVICES ──────────────────────────────────────────────────────────────
export const getTopServices = async (req: Request, res: Response): Promise<void> => {
  try {
    const periodDays = parseInt((req.query.period as string) || '30', 10);
    const since = new Date();
    since.setDate(since.getDate() - periodDays);
    ...
    const toNum = (d: any) => parseFloat(d?.toString() ?? '0');
    const map: Record<string, { service: string; commandes: number; revenus: number }> = {};

    lignes.forEach(l => {
      const key = l.service.libelle;
      if (!map[key]) map[key] = { service: key, commandes: 0, revenus: 0 };
...
export const getHourlyOrders = async (_req: Request, res: Response): Promise<void> => {
  try {
...
export const getPerformanceScore = async (req: Request, res: Response): Promise<void> => {
  try {
    const periodDays = parseInt((req.query.period as string) || '30', 10);
    const since = new Date();
    since.setDate(since.getDate() - periodDays);
