...
exports.verifyWebhookSignature = exports.getPaymentStatus = exports.extractRequestId = exports.createPaymentRequest = exports.getAccessToken = void 0;
...
const getClientId = () => {
    ...
    if (!id)
        throw new Error('NETIKASH_CLIENT_ID non configuré dans .env');
    return id;
};
const getClientSecret = () => {
    const secret = process.env.NETIKASH_CLIENT_SECRET;
    if (!secret)
        throw new Error('NETIKASH_CLIENT_SECRET non configuré dans .env');
    return secret;
};
...
const createPaymentRequest = async (params) => {
    ...
    const response = await fetch(`${API_BASE_URL}/transactions/requests/cli-payments`, {
        method: 'POST',
        headers,
...
const getPaymentStatus = async (requestId) => {
    const headers = await authHeaders();
    const response = await fetch(`${API_BASE_URL}/transactions/requests/${requestId}`, {
        method: 'GET',
        headers,
    });
    const data = await response.json();
 ...
 */
const verifyWebhookSignature = (rawBody, signatureFromHeader, signingKey) => {
    ...
    const expected = crypto_1.default
        ...
        .digest('base64');
    try {
        return crypto_1.default.timingSafeEqual(Buffer.from(expected), Buffer.from(signatureFromHeader));
    }
    catch {
        return false;
    }
