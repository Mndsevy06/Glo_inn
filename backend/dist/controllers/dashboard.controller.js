...
const getDashboardStats = async (_req, res) => {
    try {
        ...
        // Commandes du jour
        const commandesAujourdhui = await db_1.default.commande.findMany({
            where: { date_reception: { gte: today } },
        });
        const revenusJour = commandesAujourdhui.reduce((s, c) => s + parseFloat(c.montant_total.toString()), 0);
        // Commandes d'hier
        const commandesHier = await db_1.default.commande.findMany({
        ...
        // Commandes actives
        const commandesActives = await db_1.default.commande.count({
            where: { etat: { in: ['depose', 'en_cours', 'pret'] } },
        });
        const commandesPretes = await db_1.default.commande.count({
            where: { etat: 'pret' },
        });
        // Clients
        const clientCount = await db_1.default.utilisateur.count({
            where: { role: 'client' },
        });
        const nouveauxClients = await db_1.default.utilisateur.count({
            where: { role: 'client', createdAt: { gte: today } },
        });
        // Taux de paiement (toutes les commandes)
        const totalCommandes = await db_1.default.commande.count();
        const payeesCount = await db_1.default.commande.count({
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const cmds = await db_1.default.commande.findMany({
        ...
        // Status Pie
        const statusData = [
            { name: 'Prêt', value: await db_1.default.commande.count({ where: { etat: 'pret' } }), color: '#10b981' },
            { name: 'En cours', value: await db_1.default.commande.count({ where: { etat: 'en_cours' } }), color: '#6366f1' },
            { name: 'Déposé', value: await db_1.default.commande.count({ where: { etat: 'depose' } }), color: '#f59e0b' },
            { name: 'Retiré', value: await db_1.default.commande.count({ where: { etat: 'retire' } }), color: '#8b5cf6' },
        ];
        // Recent orders
        const recentOrders = await db_1.default.commande.findMany({
            take: 5,
